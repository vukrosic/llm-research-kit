"""
Run batch 9: 15 width-scaling + VR exploitation architectures, 5 min each.
"""
...
import frontier.architectures.novel_noattn_v9
...
RESULTS_DIR = ROOT / "frontier_results" / "novel_v9"
...
def main():
    ...
    experiments = sorted([k for k in list_architectures() if k.startswith('V9_')])
    print(f"V9 batch: {len(experiments)} architectures, {TIME_BUDGET}s each")
    ...
    print(f"\n{'='*70}")
    print(f" V9 RESULTS (5min)")
    print(f" Baselines: transformer=3.784, V8_13=3.684")
    print(f"{'='*70}")
    for i, r in enumerate(valid):
        d1 = r['val_loss'] - 3.784; d2 = r['val_loss'] - 3.684
        print(f" {i+1:3d}. {r['arch']:<30s} val={r['val_loss']:.4f} "
              f"(Δtrans={d1:+.3f} Δv8={d2:+.3f}) "
