"""
Run batch 10: Final exploitation + scale tests, 5 min each.
Special: V10_12 runs with seed=43 for verification.
"""
...
import frontier.architectures.novel_noattn_v10
...
RESULTS_DIR = ROOT / "frontier_results" / "novel_v10"
...
def main():
    ...
    experiments = sorted([k for k in list_architectures() if k.startswith('V10_')])
    print(f"V10 batch: {len(experiments)} architectures, {TIME_BUDGET}s each")
    ...
    for idx, arch_name in enumerate(experiments):
        ...
        # Use different seed for V10_12
        seed = 43 if 'Seed43' in arch_name else 42
        try:
            set_seed(seed)
            ...
            if seed != 42: print(f"  Using seed: {seed}")

            result = train_timed(model, train_loader, val_loader, TIME_BUDGET, tokenizer.vocab_size, seed=seed)
    ...
    print(f"\n{'='*70}")
    print(f" V10 RESULTS (5min)")
    print(f" Baselines: transformer=3.784, V9_13=3.668")
    print(f"{'='*70}")
    for i, r in enumerate(valid):
        d1 = r['val_loss'] - 3.784; d2 = r['val_loss'] - 3.668
        seed_str = f" [seed={r['seed']}]" if r.get('seed', 42) != 42 else ""
        print(f" {i+1:3d}. {r['arch']:<30s} val={r['val_loss']:.4f} "
              f"(Δtrans={d1:+.3f} Δv9={d2:+.3f}) "
              f"{r['tokens_per_sec']:,.0f} tok/s  {r['params']:,} params{seed_str}")
