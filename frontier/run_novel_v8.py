"""
Run batch 8: 15 VR+Placement exploitation architectures, 5 min each.
"""
...
import frontier.architectures.novel_noattn_v8
...
RESULTS_DIR = ROOT / "frontier_results" / "novel_v8"
...
def main():
    ...
    experiments = sorted([k for k in list_architectures() if k.startswith('V8_')])
    print(f"V8 batch: {len(experiments)} architectures, {TIME_BUDGET}s each")
    ...
    print(f"\n{'='*70}")
    print(f" V8 RESULTS (5min, d=512, L=22)")
    print(f" Baselines: transformer=3.784, V7_08_VR=3.713")
    print(f"{'='*70}")
    for i, r in enumerate(valid):
        d1 = r['val_loss'] - 3.784; d2 = r['val_loss'] - 3.713
        print(f" {i+1:3d}. {r['arch']:<30s} val={r['val_loss']:.4f} "
              f"(Δtrans={d1:+.3f} Δv7={d2:+.3f}) "
