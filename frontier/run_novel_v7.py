"""
Run batch 7: 15 SingleHead exploitation architectures, 5 min each.
"""
...
import frontier.architectures.novel_noattn_v7
...
RESULTS_DIR = ROOT / "frontier_results" / "novel_v7"
...
def main():
    ...
    experiments = sorted([k for k in list_architectures() if k.startswith('V7_')])
    print(f"V7 batch: {len(experiments)} architectures, {TIME_BUDGET}s each")
    ...
    print(f"\n{'='*70}")
    print(f" V7 RESULTS (5min, d=512, L=22)")
    print(f" Baselines: transformer=3.784, V6_06_SingleHead=3.768")
    print(f"{'='*70}")
    for i, r in enumerate(valid):
        d1 = r['val_loss'] - 3.784; d2 = r['val_loss'] - 3.768
        print(f" {i+1:3d}. {r['arch']:<30s} val={r['val_loss']:.4f} "
              f"(Δtrans={d1:+.3f} Δv6={d2:+.3f}) "
