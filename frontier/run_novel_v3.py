"""
Run batch 2: 15 enhanced non-attention architectures, 5 min each.
"""
...
import frontier.architectures.novel_noattn_v3
...
RESULTS_DIR = ROOT / "frontier_results" / "novel_v3"
...
def main():
    ...
    experiments = sorted([k for k in list_architectures() if k.startswith('V3_')])
    print(f"V2 batch: {len(experiments)} architectures, {TIME_BUDGET}s each")
    ...
    print(f"\n{'='*70}")
    print(f" V2 RESULTS (5min, d=512, L=22)")
    print(f" Baselines: transformer=3.784, PureConv=3.920, best_v1=4.004, best_v2=TBD")
    print(f"{'='*70}")
    for i, r in enumerate(valid):
        d1 = r['val_loss'] - 3.784; d2 = r['val_loss'] - 3.920
        print(f" {i+1:3d}. {r['arch']:<25s} val={r['val_loss']:.4f} "
              f"(Δtrans={d1:+.3f} Δconv={d2:+.3f}) "
